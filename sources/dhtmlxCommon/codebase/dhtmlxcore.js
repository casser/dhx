    ...
    if (coll) {
        ...
        for (var i = 0; i < coll[mode].length; i++) {
            ...
            if (typeof rule == "function")
                check = check || rule(value); else if (typeof rule == "object" && typeof rule[1] == "function") {
...
dhtmlx.proto_extend = function () {
    ...
    for (var i = origins.length - 1; i > 0; i--) {
        ...
        for (var key in origins[i]) {
            if (key == "_init")
                construct.push(origins[i][key]); else if (!compilation[key])
...
dhtmlx.html = {
    ...
    pos: function (ev) {
        ...
        //IE
        var d = (dhx4.isIE && (document.compatMode != "BackCompat")) ? document.documentElement : document.body;
        return {
...
dhtmlx.DataDriver.xml = {
    ...
    records: "/*/item", //get hash of properties for single record
    getDetails: function (data) {
    ...
    fromString: function (xmlString) {
        if (window.DOMParser && !dhx4.isIE)		// FF, KHTML, Opera
...
dhtmlx.DataStore.prototype = {
        ...
        for (var i = 0; i < recs.length; i++) {
            ...
            if (this._scheme) {
                if (this._scheme.$init)
                    this._scheme.$update(temp); else if (this._scheme.$update)
    ...
    refresh: function (id) {
        ...
        if (id)
            this.callEvent("onStoreUpdated", [id, this.pull[id], "update"]); else
    ...
    add: function (obj, index) {
        if (this._scheme) {
            ...
            if (this._scheme) {
                if (this._scheme.$init)
                    this._scheme.$update(obj); else if (this._scheme.$update)
...
dhtmlx.MouseEvents = {
    _init: function () {
        ...
        if (this.on_mouse_move) {
            ...
            dhtmlx.event(this._obj, (dhx4.isIE ? "mouseleave" : "mouseout"), this._onMouse, this);
        }
     ...
     */
    _onMouse: function (e) {
        if (dhx4.isIE)	//make a copy of event, will be used in timed call
