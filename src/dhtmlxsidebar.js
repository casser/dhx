class dhtmlXSideBar extends dhtmlXCellTop {
    constructor(conf) {
        super();
        var that = this;
        this.conf = {
            ...
            selected: null, // single cell mode, added in 4.3
            single_cell: (typeof(conf.single_cell) == "undefined" ? false : window.dhx4.s2b(conf.single_cell)),
            cell: null, // cells header, added in 4.5
            header: window.dhx4.s2b(conf.header), // autohide side, added in 4.5
        ...
        // init base
        this._init(conf.parent, (conf == null ? null : conf.offsets));
        // common event system
        ...
        this._doOnArwClick = function (e) {
			...
			if (t.className.match(/dhxsidebar_arrow_image/) != null) {
				t = t.parentNode;
			}
        ...
        this._scrollSide = function (dir) { // dir => -1/1
            ...
            // first check down
			if (top + this.side.firstChild.offsetHeight < this.side.clientHeight) {
				top = this.side.clientHeight - this.side.firstChild.offsetHeight;
			}
			// also check top
			if (top > 0) {
				top = 0;
        ...
        this._adjustCell = function (id, force) {
			if (this.conf.single_cell != true && id == null) {
				return;
			}
            ...
            if (this.conf.single_cell == true) {
				if (force == true) {
					this.conf.cell._setSize(x, y, w, h);
				} // only call from setSizes
            } else {
...
dhtmlXSideBar.prototype._addItem = function (conf) {
	...
	if (this.conf.header == true) {
		cell.setHeaderText(window.dhx4.template(this.tpl_header, conf));
	}
	cell = t = null;
...
dhtmlXSideBar.prototype.goToNextItem = function (callEvent) {
	...
	if (id != null) {
		this._setItemActive(id, callEvent);
	}
};
...
dhtmlXSideBar.prototype.setTemplate = function (template, iconsPath) {
    ...
    // icons path if any
	if (iconsPath != null) {
		this.conf.icons_path = iconsPath;
	}
	// update loaded items
    for (var a in this.t) {
...
class dhtmlXSideBarCell extends dhtmlXCellObject {
    constructor(id, sidebar) {
        ...
        this.attachEvent("_onCellUnload", function () {
            ...
            // sidebar calls
            for (var a in this.conf.sidebar_funcs) {
                this[a] = this.conf.sidebar_funcs[a] = null;
            }
...
dhtmlXSideBar.prototype._getItemText = function (id) {
    var t = {};
    if (this.t[id] != null) {
		for (var a in this.t[id].init) {
			t[a] = this.t[id].init[a];
		}
	}
    return t;
}
...
dhtmlXSideBar.prototype._showItem = function (id, actv) {
	if (this.t[id] == null || this.t[id].conf.visible == true || this.t[id].conf.transActv == true) {
		return;
	}
	if (this.conf.transProp !== false) {
...
dhtmlXSideBarCell.prototype._initHeader = function () {
    ...
    t.innerHTML = (this.sidebar.conf.autohide == true ? "<div class='dhx_cell_sidebar_hdr_icon'></div>" : "") + "<div class='dhx_cell_sidebar_hdr_text" + (this.sidebar.conf.autohide == true ? " dhx_cell_sidebar_hdr_text_icon" : "") + "'></div>";
