					...
					for (var w = 0; w < this.tpl.length; w++) {
						...
						if (this._IEFirstLast && (this.tpl.length == 1 || w == 0 || w == this.tpl.length - 1)) {
							if (this.tpl.length == 1) {
								css += " dhx_popup_td_single";
							} else {
								css += (w == 0 ? " dhx_popup_td_first" : " dhx_popup_td_last");
							}
						}
						var td = document.createElement("TD");
						td.className = css;
		...
		if (this._IEHover) {
			this._IEHoverInit = function () {
				this.p.onmouseover = function () {
					...
					while (t != this && t._IEHover != true) {
						t = t.parentNode;
					}
					if (t._IEHover) {
						if (that._IEHoverTM) {
							window.clearTimeout(that._IEHoverTM);
						}
						if (that._lastIEHover == t) {
							return;
						}
