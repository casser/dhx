class dhtmlXCarousel extends dhtmlXCellTop {
    static get config(){
    ...
    get ef(){
        return dhtmlXCarousel.config.ef;
    }
    constructor(conf, effect, skin) {
        super()
        ...
        if (this.ef[this.conf.anim_type] == true) {
            ...
            if (typeof(t) == "object") {
                for (var a in t) {
					if (typeof(this.conf[a]) == "undefined") {
						this.conf[a] = t[a];
					}
				}
                ;
        ...
        var that = this;
        this._init(conf.parent, conf.offsets)
        ...
        if (typeof(window.addEventListener) == "function" && that.conf.touch_scroll == true) {
            this._doOnTouchStart = function (e) {
				if (window.dhx4.dnd._mTouch(e) == true) {
					return;
				}
				if (that.conf.animating == true) {
					return;
				}
				if (e.preventDefault) {
					e.preventDefault();
            ...
            this._doOnTouchEnd = function (e) {
				...
				if (ofsX == 0 || new Date().getTime() - that.conf.touch_conf.t > 400) {
					return;
				}
        ...
        this.setSizes = function () {
            ...
            for (var a in this.cdata) {
                var s = {};
				for (var b in this.cdata[a].cell.conf.size) {
					s[b] = this.cdata[a].cell.conf.size[b];
				}
				//
        ...
        this._animateStart = function (dir, ef) {
			if ((this.conf.selected <= 0 && dir < 0) || (this.conf.selected >= this.conf.items_count - 1 && dir > 0)) {
				return;
			}
        ...
        this.setOffset = function (left, top, item) {
			if (left != null) {
				this.conf.ofs_left = left;
			}
			if (top != null) {
				this.conf.ofs_top = top;
			}
			if (item != null) {
				this.conf.ofs_item = item;
			}
			this.setSizes();
        }
    ...
    _initControls() {
        ...
        this.controls.innerHTML = "<div class='dhx_carousel_bars'></div>" + "<div class='dhx_carousel_btn dhx_carousel_btn_prev'>" + a[0] + "</div>" + "<div class='dhx_carousel_btn dhx_carousel_btn_next'>" + a[1] + "</div>";
        ...
        this._doOnControlClick = function (e) {
            ...
            if (window.dhx4.dnd.evs.start != null && e.type != "click" && that.conf.clear_click != true) { // force animation on touch devices by touch
                if (window.dhx4.dnd._mTouch(e) == true) {
                    return;
                } // prevent pointer on desktop in IE
            ...
            if (t.className != null) {
                if (t.className.match(/btn_prev/) != null) {
                    ...
                    for (var q = 0; q < t.parentNode.childNodes.length; q++) {
                        if (t.parentNode.childNodes[q] == t) {
                            anim = q - that.conf.selected;
                        }
                    }
                }
            }
            if (anim != null) {
                that._animateStart(anim);
            }
            t = null;
        }
        if (typeof(window.addEventListener) == "function") {
    ...
    _slide_start(step, curX, maxX, dir) {
        var stop = false;
        curX += step;
        if (curX >= maxX) {
            curX = maxX;
    ...
    _cards_start(step, curX, maxX, dir) {
        // old browsers?
    };
    _cards_end(e, obj) {
    ...
    _cards_update_selected(id) {
        this.cdata[id].cell._hideCover();
    };
    _cards_adjust_middle(fromIndex, toIndex, i) {
    ...
    _cards_detect_area_width() {
        return 2;
    };
}
class dhtmlXCarouselCell extends dhtmlXCellObject {
    ...
    setActive() {
        ...
        if (ofs != 0) {
            this.carousel._animateStart(ofs);
        }
    }
    remove() {
