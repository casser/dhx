class dhtmlXMenuObject {
	static get config(){
		return Object.defineProperty(this,'config',{
			value : {
				liveInst:{}
			}
		}).config;
	...
	get liveInst(){
		return dhtmlXMenuObject.config.liveInst;
	}
    constructor(base, skin) {
        ...
        this.conf = {
            ...
            autoload: {},
            hide_tm: {}, // shows sublevel polygons from toplevel items with delay
            top_mode: true,
            top_tmtime: 200, // visible area
            v_enabled: false,
            v: {
                x1: null,
                x2: null,
        ...
        if (typeof(base) == "object" && base != null && typeof(base.tagName) == "undefined") {
            ...
            // object-api init
            if (base.icons_path != null || base.icon_path != null) {
                this.conf.icons_path = (base.icons_path || base.icon_path);
            }
            if (base.skin != null) {
                this.conf.skin = base.skin;
            }
        ...
        this._showPolygon = function (id, openType) {
            ...
            if ((this.idPull[pId] != null) && (this.idPull[id] != null)) {
                if (this.conf.top_mode && this.conf.mode == "web" && !this.conf.context) {
                    if (!this.idPull[id]._mouseOver && openType == this.conf.dir_toplv) {
                        return;
                    }
                }
                // detect visible area
                ...
                if (this.conf.auto_overflow) {
                    if (this.idPull[pId].childNodes[1].childNodes[0].offsetHeight > this.conf.v.y2 - this.conf.v.y1) {
                        ...
                        if (this.idPull["arrowup_" + id] != null) {
                            this._removeUpArrow(String(id).replace(this.idPrefix, ""));
                        }
                        if (this.idPull["arrowdown_" + id] != null) {
                            this._removeDownArrow(String(id).replace(this.idPrefix, ""));
                        }
                    }
                }
            ...
            if (this.itemPull[id]["state"] == "enabled") {
                ...
                if (this.itemPull[id]["complex"] && this.conf.dload && (this.itemPull[id]["loaded"] == "no")) {
                    ...
                    this._dhxdataload.onBeforeXLS = function () {
                        ...
                        for (var a in this.conf.dload_params) {
                            p.params[a] = this.conf.dload_params[a];
                        }
                        return p;
                    };
        ...
        this._doOnClick = function (id, type, casState) {
            ...
            if (type.charAt(0) == "c") {
                return;
            } // can't click on complex item
            if (type.charAt(1) == "d") {
                return;
            } // can't click on disabled item
            if (type.charAt(2) == "s") {
                return;
            } // can't click on separator
            //
            if (this.checkEvent("onClick")) {
                this.callEvent("onClick", [id, this.conf.ctx_zoneid, casState]);
            } else {
                if ((type.charAt(1) == "d") || (this.conf.mode == "win" && type.charAt(2) == "t")) {
                    return;
                }
            }
        ...
        this._clearAndHide = function () {
            ...
            for (var q = 0; q < that.conf.selected_sub.length; q++) {
                ...
                if (that.idPull[id] != null) {
                    if (that.itemPull[id]["state"] == "enabled") {
                        if (that.idPull[id].className == "sub_item_selected") {
                            that.idPull[id].className = "sub_item";
                        }
            ...
            if (that.conf.context && that.conf.ctx_hideall) {
                that._hidePolygon(that.idPrefix + that.topId);
            }
        }
        /* show sublevel item */
        ...
        this._bodyContext = function (e) {
            ...
            while (testZone != null) {
                if (testZone.id != null) {
                    if (that.isContextZone(testZone.id)) {
                        toHide = false;
                    }
                }
	...
	_updateItemComplexState(id, state, over) {
	...
	_redistribTopLevelSelection(id, parent) {
	...
	_addSubMenuPolygon(id, parentId) {
	...
	_renderSublevelPolygon(id, parentId) {
	...
	_initObj(items, nested, parentId) {
	...
	_addItemIntoGlobalStrorage(itemId, itemParentId, itemText, itemType, disabled, img, imgDis) {
		var item = {
			id:	itemId,
			title:	itemText,
			imgen:	(img!=null?img:""),
	...
	_doOnContextBeforeCall(e, cZone) {
	...
	_improveTerraceButton(id, state) {
		if (state) {
...
XCellObject.prototype._createNode_menu = function(obj, type, htmlString, append, node) {
...
XCellObject.prototype.attachMenu = function(conf) {
...
XCellObject.prototype.detachMenu = function() {
...
XCellObject.prototype.showMenu = function() {
	this._mtbShowHide("menu", "");
};
XCellObject.prototype.hideMenu = function() {
	this._mtbShowHide("menu", "none");
};
XCellObject.prototype.getAttachedMenu = function() {
	return this.dataNodes.menu;
};
