...
dhtmlXList.prototype = {
            ...
            if (this.stopEdit(false, id)) {
                if (this._settings.select) {
					if (this._settings.select == "multiselect") {
						this.select(id, e.ctrlKey, e.shiftKey);
					}//multiselection
					else {
						this.select(id);
					}
    ...
    types: {
        "default": {
            ...
            template_edit: dhtmlx.Template.fromHTML("<div style='padding:10px; white-space:nowrap; overflow:hidden;'><textarea bind='obj.text'></textarea></div>"), //in case of dyn. loading - temporary spacer
...
XCellObject.prototype.attachList = function (conf) {
