...
dhtmlXGridObject.prototype._createDragNode = function (htmlObject, e) {
    this.editStop();
    if (dhtmlDragAndDropObject.dhtmlDragAndDrop.dragNode) return null;
    ...
    var el = this.getFirstParentOfType(dhx4.isIE ? e.srcElement : e.target, "TD");
...
dhtmlXGridObject.prototype._drag = function (sourceHtmlObject, dhtmlObject, targetHtmlObject, lastLanding) {
    ...
    if (r2 && r2.childNodes)
        c.set("source", "tree").set("sobj", r2.treeNod).set("sid", c.sobj._dragged); else {
        if (!r2) return true;
    ...
    if (((c.tobj.dadmode == 2) && (c.tobj.dadmodec == 1)) && (c.tobj.dadmodefix < 0))
        if (c.tobj.obj.rows[1].idd != c.tid) c.tid = r1.previousSibling.idd; else {
            if (this._h2 && c.tid)
    ...
    if (r2 && r2.treeNod && r2.treeNod._nonTrivialRow)
        r2.treeNod._nonTrivialRow(this, c.tid, c.dropmode, r2); else {
...
dhtmlXGridObject.prototype._dragRoutine = function (c) {
    ...
    if (this._h2 && typeof c.tid != "undefined" && c.dropmode == "sibling" && (this._sbmod || c.tid)) {
        if (c.before)
            new_row = c.uid().tobj.addRowBefore(c.nid, c.data(), c.tid, c.img(), c.childs()); else if (c.alfa && this._sbmod && this._h2.get[c.tid].childs.length) {
            this.openItem(c.tid);
...
dhtmlXGridObject.prototype._copyUserData = function (c) {
    if (!c.tobj.UserData[c.nid] || c.tobj != c.sobj)
        c.tobj.UserData[c.nid] = new Hashtable(); else return;
...
dhtmlXGridObject.prototype._nonTrivialNode = function (tree, targetObject, beforeNode, itemObject, z2) {
    ...
    if (beforeNode) {
        ...
        if (i != 0)
            beforeNode = targetObject.childNodes[i - 1]; else {
            st = "TOP";
            beforeNode = "";
        }
    }
