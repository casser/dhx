(function () {
    ...
    dhx.protoUI = function () {
        ...
        var t = function (data) {
            ...
            if (this != dhx)
                return new dhx.ui[selfname](data); else
    ...
    dhx.event = function (node, event, handler, master) {
        ...
        if (node.addEventListener)
            node.addEventListener(event, handler, false); else if (node.attachEvent)
    ...
    dhx.eventRemove = function (id) {
        ...
        if (ev[0].removeEventListener)
            ev[0].removeEventListener(ev[1], ev[2], false); else if (ev[0].detachEvent)
    ...
    dhx.log = function (type, message, details) {
        ...
        if (window.console && console.log) {
            ...
            if (window.console[type])
                window.console[type](message || "unknown error"); else
    ...
    dhx.html = {
        ...
        addStyle: function (rule) {
            ...
            /*IE8*/
            if (style.styleSheet)
                style.styleSheet.cssText = rule; else
        ...
        insertBefore: function (node, before, rescue) {
            ...
            if (before && before.parentNode)
                before.parentNode.insertBefore(node, before); else
    ...
    dhx.ready = function (code) {
        if (this._ready) code.call(); else this._ready_code.push(code);
    };
    dhx._ready_code = [];
    (function () {
        ...
        dhx.Type = function (obj, data) {
            ...
            for (var key in data) {
                if (key.indexOf("template") === 0)
                    type[key] = dhx.Template(data[key]); else
                    type[key] = data[key];
            }
            return name;
        };
    ...
    dhx.ajax.count = 0;
    dhx.ajax.prototype = {
        master: null, //creates xmlHTTP object
        getXHR: function () {
            if (dhx.env.isIE)
    ...
    dhx.AtomDataLoader = {
        ...
        debug_freid_c_dataFeed: true, //loads data from external URL
        load: function (url, call) {
            ...
            if (call) {
                if (dhx.isArray(call))
                    callback.push.apply(callback, call); else
                    callback.push(call);
            }
        ...
        _check_data_feed: function (data) {
            ...
            dhx.ajax(url, function (text, xml, loader) {
                ...
                if (data)
                    this.setValues(data.getDetails(data.getRecords()[0])); else
                    this._onLoadError(text, xml, loader);
    ...
    dhx.DataDriver.xml = {
        ...
        toObject: function (text, xml) {
            ...
            if (typeof text == "string")
                var data = this.fromString(text.replace(/^[\s]+/, "")); else
                data = text;
            if (this._isValidXML(data))
                return data;
            return null;
        ...
        records: "/*/item",
        child: "item",
        config: "/*/config", //get hash of properties for single record
        getDetails: function (data) {
        ...
        getInfo: function (data) {
            ...
            if (config.length)
                config = this.assignTypes(this.tagToObject(config[0], {})); else
                config = null;
            return {
        ...
        xpath: function (xml, path) {
            if (window.XPathResult) {	//FF, KHTML, Opera
                ...
                //IE
                if (test)
                    return xml.selectNodes(path); else {
        ...
        assignTypes: function (obj) {
            for (var k in obj) {
                ...
                if (typeof test == "object")
                    this.assignTypes(test); else if (typeof test == "string") {
                    if (test === "")
                        continue;
                    if (test == "true")
                        obj[k] = true; else if (test == "false")
                        obj[k] = false; else if (test == test * 1)
                        obj[k] = obj[k] * 1;
                }
            }
            return obj;
    ...
    dhx.DataStore.prototype = {
        ...
        refresh: function (id) {
            ...
            if (id)
                this.callEvent("onStoreUpdated", [id, this.pull[id], "paint"]); else
        ...
        getRange: function (from, to) {
            ...
            if (from)
                from = this.indexById(from); else
                from = (this.$min || this.startOffset) || 0;
            if (to)
                to = this.indexById(to); else {
        ...
        sync: function (source, filter, silent) {
            ...
            if (this._backbone_source)
                source.bind('all', sync_logic); else
    ...
    dhx.BindSource = {
        ...
        setBindData: function (data, key) {
            ...
            if (this.setValue)
                this.setValue(data); else if (this.setValues)
                this.setValues(data); else {
        ...
        _bind_specific_rules: function (obj) {
            if (obj.filter)
                dhx.extend(this, dhx.CollectionBind); else if (obj.setValue)
                dhx.extend(this, dhx.ValueBind); else
        ...
        _bind_update_common: function (target, rule, data) {
            if (target.setValue)
                target.setValue(data ? data[rule] : data); else if (!target.filter) {
    ...
    dhx.ValueBind = {
        ...
        _bind_update: function (target, rule, format) {
            ...
            if (target.setValue)
                target.setValue(data); else if (!target.filter) {
                var pod = {};
                pod[rule] = data;
                if (target._check_data_feed(data))
    ...
    if (dhtmlXForm) {
        ...
        dhtmlXForm.prototype.dataFeed = function (value) {
            if (this._settings)
                this._settings.dataFeed = value; else
            ...
            scheduler.sync = function (source, rule) {
                ...
                var insync = function (ignore) {
                    ...
                    for (var i = 0; i < order.length; i++) {
                        if (rule && rule.copy)
                            evs[i] = dhx.clone(pull[order[i]]); else
                            evs[i] = pull[order[i]];
                    }
        ...
        dhtmlXGridObject.prototype.sync = function (source, rule) {
            ...
            var insync = function (ignore) {
                ...
                if (count) {
                    ...
                    if (grid.pagingOn)
                        grid.changePage(); else {
                        if (grid._srnd && grid._fillers)
                            grid._update_srnd_view(); else {
                            grid.render_dataset();
