class dhtmlXRibbon {
    constructor(struct) {
        ...
        this._tabCustomApi = {
            ...
            close: function (actvId) {
                ...
                for (var key in that._tabOriginalApi) {
                    this[key] = null;
                }
            }
        };
    ...
    _xmlToJson(xml) {
        var root = xml.lastChild || null, answer = {}, list = [];
    ...
    _convertXmlNodeListIntoObject(nodeList) {
        var i, l, answer = [], item;
        l = nodeList.length;
        for (i = 0; i < l; i++) {
    ...
    _addItem(blockId, nextToId, itemId, data) {
        ...
        if (base) {
            ...
            ;
            if (!itemData.id) {
                itemData.id = dhx4.newId();
            }
            while (this._items[itemData.id]) {
                itemData.id = dhx4.newId();
            }
    ...
    _getContainerForSmallItem(parent) {
    ...
    check(id, callEvent) {
        ...
        if (item && !item.conf.checked && typeof(this.items[item.type].check) == "function") {
            ...
            if (callEvent) {
                this.callEvent("onCheck", [item.id, item.conf.checked]);
            }
        }
    }
    uncheck(id, callEvent) {
        callEvent = dhx4.s2b(callEvent);
        var item = this._items[id];
    ...
    setItemState(id, value, callEvent) {
    ...
    setItemOptionText(id, optId, text) {
        var item = this._items[id];
        if (item.type == "buttonSelect") {
            this.items[item.type].setOptionText(item, optId, text);
        }
    }
    getItemOptionText(id, optId) {
        var item = this._items[id];
        if (item.type == "buttonSelect") {
            return this.items[item.type].getOptionText(item, optId);
        }
        return null;
    }
    setItemImage(id, img) {
    ...
    getCombo(id) {
        var item = this._items[id];
        if (item != null && item.type == "buttonCombo") {
            return item.combo;
        }
        return null;
    }
}
class dhtmlXRibbonButton {
    render(cont, itemData) {
        var key, item;
        item = {
            base: cont,
            id: itemData.id,
    ...
    disable(item) {
        var contForImage = item.base.childNodes[0], contForText = item.base.childNodes[1];
        if (item.conf.imgdis) {
...
class dhtmlXRibbonTwoStateButton extends dhtmlXRibbonButton {
    click(item, rb, e) {
        return false;
    }
...
class dhtmlXRibbonButtonSegment extends dhtmlXRibbonTwoStateButton {
    click(item, rb, e) {
        return false;
    }
    ...
    remove(item, rb) {
        var state = dhx4.s2b(item.conf.state), firstItem = null, i = 0, parent = rb._items[item.parentId], tempItem;
        if (state) {
...
class dhtmlXRibbonButtonSelect extends dhtmlXRibbonTwoStateButton {
    constructor() {
        super();
        this.itemCollection = [];
    ...
    getOptionText(item, optId) {
        if (item.menu != null) {
            return item.menu.getItemText(optId);
        }
        return this._loopThroughItems(item.conf.items, optId, null, true); // loop through items
    }

    _loopThroughItems(items, id, text, retValue) {
        for (var q = 0; q < items.length; q++) {
            if (items[q].id == id) {
                if (retValue != true) {
    ...
    showMenu(item) {
        var x = dhx4.absLeft(item.base), y = dhx4.absTop(item.base) + item.base.offsetHeight;
        if (!(item.menu instanceof dhtmlXMenuObject)) {
            ...
            item.menu.attachEvent("onHide", function (id) {
                if (id == null) {
                    dhtmlXRibbon.prototype.items.buttonSelect._doOnHideMenu(item);
                }
            });
            item.menu.attachEvent("onShow", function (id) {
                if (id == null) {
                    dhtmlXRibbon.prototype.items.buttonSelect._doOnShowMenu(item);
                }
            });
...
class dhtmlXRibbonButtonInput {
    ...
    disable(item) {
        var contForInput = item.base.childNodes[0], contForText = item.base.childNodes[1];
...
class dhtmlXRibbonButtonCheckbox {
    render(cont, itemData) {
        var key, item;
        item = {
            base: cont,
...
class dhtmlXRibbonButtonButtonCombo {
    render(cont, dataItem) {
...
class dhtmlXRibbonButtonSlider {
    render(cont, data) {
...
dhtmlXRibbon.prototype.items = {
    button: new dhtmlXRibbonButton(),
    buttonTwoState: new dhtmlXRibbonTwoStateButton(),
    buttonSegment: new dhtmlXRibbonButtonSegment(),
    buttonSelect: new dhtmlXRibbonButtonSelect(),
    group: new dhtmlXRibbonButtonGroup(),
    input: new dhtmlXRibbonButtonInput(),
    checkbox: new dhtmlXRibbonButtonCheckbox(),
    text: new dhtmlXRibbonButtonText(),
    buttonCombo: new dhtmlXRibbonButtonButtonCombo(),
    slider: new dhtmlXRibbonButtonSlider()
};
...
XCellObject.prototype._createNode_ribbon = function (obj, type, htmlString, append, node) {
...
XCellObject.prototype.attachRibbon = function (conf) {
...
XCellObject.prototype.detachRibbon = function () {
...
XCellObject.prototype.showRibbon = function () {
    this._mtbShowHide("ribbon", "");
};
XCellObject.prototype.hideRibbon = function () {
    this._mtbShowHide("ribbon", "none");
};
XCellObject.prototype.getAttachedRibbon = function () {
    return this.dataNodes.ribbon;
};
