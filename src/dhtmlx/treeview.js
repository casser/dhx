class dhtmlXTreeView {
	...
	_refreshItemHtml(id, updSign, updIcon) {
	...
	_chbxGetCheckedBranch(pId, mode) {
		var k = [];
		for (var a in this.items) {
	...
	_itemHtml_sign(id, nodeIndex) { // item html renderer
		return {
			nodeIndex: 1,
	...
	_iconUpdate(id) { // update icon inner call
		var t = this.items[id];
...
XCellObject.prototype.attachTreeView = function(conf) {
