...
dhtmlx.ui.pager.prototype = {
    ...
    select: function (id) {
		...
		if (id >= this.data.limit) {
			id = this.data.limit - 1;
		}
    ...
    types: {
        "default": {
            template: dhtmlx.Template.fromHTML("{common.pages()}"), //list of page numbers
            pages: function (obj) {
                var html = "";
...
dhtmlx.DataMove = {
    ...
    move: function (sid, tindex, tobj, tid) {
		...
		if (!tobj || tobj == this) {
			this.data.move(this.indexById(sid), tindex);
...
dhtmlx.DragControl = {
    ...
    _startDrag: function (e) {
		...
		dhtmlx.DragControl._preStartFalse();
		if (!dhtmlx.DragControl.createDrag(e)) {
			return;
		}
    ...
    _checkLand: function (node, e) {
        while (node && node.tagName != "BODY") {
            if (node.dhx_drop) {	//if drop area registered
				...
				{
					this.onDragOut(this._active, this._last, node, e);
				}			//inform master about possible mouse-out
    ...
    left: 5, //called when mouse was moved in drop area
    onDragIn: function (s, t, e) {
        var m = this._drag_masters[t.dhx_drop];
		if (m.onDragIn && m != this) {
			return m.onDragIn(s, t, e);
		}
    ...
    onDragOut: function (s, t, n, e) {
        var m = this._drag_masters[t.dhx_drop];
		if (m.onDragOut && m != this) {
			return m.onDragOut(s, t, n, e);
		}
...
dhtmlx.DragItem = {
    ...
    onDragIn: function (s, t, e) {
		...
		if (html == dhtmlx.DragControl._landing) {
			return html;
		}
		context.target = id;
        context.to = to;
...
dhtmlx.EditAbility = {
    ...
    stopEdit: function (mode, if_not_id) {
        ...
        var values = {};
        if (!mode) this._edit_bind(false, values); else values = null;
...
dhtmlx.SelectionModel = {
    ...
    _data_updated: function (id, obj, type) {
        if (type == "delete")				//remove selection from deleted items
            this._selected.remove(id); else if (!this.data.dataCount() && !this.data._filter_order) {	//remove selection for clearAll
            this._selected = dhtmlx.toArray();
        }
    },
    ...
    _render_visible_rows: function (e, reset) {
        ...
        while (t <= viewport._height) {	//loop for all visible rows
            ...
            if (delta <= 0 && delta2 > 0) {
                ...
                if (delta < 0)
                    dhtmlx.html.remove(holder_row); else
...
dhtmlXDataView.prototype = {
    ...
    _id: "dhx_f_id", //css class to action map, for onclick event
    on_click: {
    ...
    types: {
        "default": {
            css: "default", //normal state of item
            template: dhtmlx.Template.fromHTML("<div style='padding:10px; white-space:nowrap; overflow:hidden;'>{obj.text}</div>"), //template for edit state of item
            template_edit: dhtmlx.Template.fromHTML("<div style='padding:10px; white-space:nowrap; overflow:hidden;'><textarea style='width:100%; height:100%;' bind='obj.text'></textarea></div>"), //in case of dyn. loading - temporary spacer
...
XCellObject.prototype.attachDataView = function (conf) {
