class XCellObject {
    constructor(idd, css) {
        ...
        this._adjustAttached = function () {
            ...
            if (this.dataObj != null && typeof(this.dataObj.setSizes) == "function") {
                ...
                if (this.dataType == "layout" && typeof(XLayoutCell) == "function" && this instanceof XLayoutCell && this.dataObj._getMainInst() != this.layout._getMainInst()) {
        ...
        this._setSize = function (x, y, w, h, parentIdd, noCalcCont, actionType, customProps) {
            ...
            if (customProps == null) {
                customProps = {};
            } // ability to use margin-left instead of left
            var styleProps = {
                left: "x",
                top: "y",
                width: "w",
        ...
        this._adjustCont = function (parentIdd, actionType) {
            ...
            if (typeof(XLayoutCell) == "function" && this instanceof XLayoutCell && this.conf.collapsed == true) {
        ...
        this._viewRestore = function (name) {
            ...
            for (var q = 0; q < this.views[name].cellCont.length; q++) {
                this.cell.childNodes[this.conf.idx.cont].appendChild(this.views[name].cellCont[q]);
            }
            ...
            for (var a in this.views[name].dataNodes) {
                ...
                if (a == "menu") {
                    this.cell.insertBefore(this.views[name].dataNodesCont[a], this.cell.childNodes[this.conf.idx.toolbar || this.conf.idx.cont]);
                }
                if (a == "toolbar") {
                    this.cell.insertBefore(this.views[name].dataNodesCont[a], this.cell.childNodes[this.conf.idx.cont]);
                }
                if (a == "ribbon") {
                    this.cell.insertBefore(this.views[name].dataNodesCont[a], this.cell.childNodes[this.conf.idx.cont]);
                }
                if (a == "sb") {
                    this.cell.appendChild(this.views[name].dataNodesCont[a]);
                }
                this._updateIdx();
            }
            // ofs_nodes
            for (var a in this.views[name].ofs_nodes) {
                for (var b in this.views[name].ofs_nodes[a]) {
                    this.conf.ofs_nodes[a][b] = this.views[name].ofs_nodes[a][b];
                }
            }
        ...
        this._viewDelete = function (name) {
            ...
            for (var a in this.views[name].ofs_nodes) {
                for (var b in this.views[name].ofs_nodes[a]) {
                    this.views[name].ofs_nodes[a][b] = null;
                }
            ...
            for (var q = 0; q < this.views[name].cellCont.length; q++) {
                this.views[name].cellCont[q] = null;
            }
    ...
    unloadView(name) {
        ...
        for (var a in v.dataNodes) {
            if (typeof(v.dataNodes[a].unload) == "function") {
                v.dataNodes[a].unload();
            }
            v.dataNodes[a] = null;
    ...
    attachURL(url, useAjax, postData) {
        ...
        if (useAjax == true) {
            ...
            if (postReq) {
                ...
                for (var a in postData) {
                    params += "&" + encodeURIComponent(a) + "=" + encodeURIComponent(postData[a]);
                }
                this.conf.url_data.xml_doc = dhx4.ajax.post(url, params, function (r) {
                    if (t.attachHTMLString != null && typeof(r.xmlDoc.responseText) == "string") {
                        ...
                        if (typeof(t._doOnFrameContentLoaded) == "function") {
    ...
    _createNode_sb(obj, type, htmlString, append, node) {
        ...
        if (typeof(node) != "undefined") {
            ...
            // height, optional
            if (h != false) {
                obj.firstChild.style.height = obj.firstChild.style.lineHeight = h + "px";
            }
        }
    ...
    attachStatusBar(conf) { // args-optinal, new in version
    ...
    _getFrame() { // _getFrame deprecated, use getFrame
        if (this.dataType != "url") {
            return null;
        }
        return this.cell.childNodes[this.conf.idx.cont].firstChild;
    ...
    _attachURLEvents() {
        ...
        if (typeof(window.addEventListener) == "function") {
            fr.onload = function () {
                try {
                    if (typeof(t._doOnFrameMouseDown) == "function") {
                        this.contentWindow.document.body.addEventListener("mousedown", t._doOnFrameMouseDown, false);
                    }
                } catch (e) {
                }
                try {
            ...
            fr.onreadystatechange = function (a) {
                if (this.readyState == "complete") {
                    try {
                        if (typeof(t._doOnFrameMouseDown) == "function") {
                            this.contentWindow.document.body.attachEvent("onmousedown", t._doOnFrameMouseDown);
                        }
                    } catch (e) {
                    }
                    try {
    ...
    _doOnFrameContentLoaded() {
        if (this.conf.url_data.post_ifr == true) {
            ...
            for (var a in this.conf.url_data.post_data) {
                postData[a] = this.conf.url_data.post_data[a];
            }
            for (var a in postData) {
    ...
    _attachObject(obj, type, htmlString, append, node) {
        if (typeof(obj) == "string" && {
                menu: 1,
                toolbar: 1,
                ribbon: 1,
    ...
    _detachObject(obj, remove, moveTo) {
    ...
    _attachFromCell(cell) {
        ...
        if (typeof(dhtmlXWindowsCell) == "function" && cell instanceof dhtmlXWindowsCell && cell.wins.w[cell._idd].conf.parked == true) {
            cell.wins._winCellSetOpacity(cell._idd, "open", false);
        }
        // 2) acc-cell collapsed
        if (typeof(dhtmlXAccordionCell) == "function" && cell instanceof dhtmlXAccordionCell && cell.conf.opened == false) {
...
class XCellTop {
    constructor(base, offsets) {
        if (typeof(base) != "undefined") {
            this._init(base, offsets)
        }
    }
    _init(base, offsets) {
        var that = this;
        ...
        this._adjustCont = function () {
            ...
            for (var a in this.conf.ofs_nodes.t) {
                ofsYT += (this.conf.ofs_nodes.t[a] == true ? this.dataNodes[a].offsetHeight : 0);
            }
            var ofsYB = this.conf.ofs.b;
            for (var a in this.conf.ofs_nodes.b) {
                ofsYB += (this.conf.ofs_nodes.b[a] == true ? this.dataNodes[a].offsetHeight : 0);
            }
    ...
    setOffsets(data, upd) { // set 'upd' to false to prevent adjusting (useful on init stage)
        var t = false;
        for (var a in data) {
            var k = a.charAt(0);
        ...
        if (upd !== false && typeof(this.setSizes) == "function" && t == true) {
            this.setSizes();
        }
    }
    attachMenu(conf) {
    ...
    attachStatusBar(conf) { // arg-optional, new in version
        ...
        if (typeof(conf.height) == "number") {
            this.dataNodes.sbObj.firstChild.style.height = this.dataNodes.sbObj.firstChild.style.lineHeight = conf.height + "px";
        }
    ...
    _mtbShowHide(name, disp) {
        if (this.dataNodes[name] == null) {
            return;
        }
        this.dataNodes[name].style.display = disp;
        this.setSizes();
    }
    _mtbUnload(name, disp) {
        this.detachMenu();
        this.detachToolbar();
        this.detachStatusBar();
        this.detachRibbon();
