...
function dhtmlXGridObject(id) {
    ...
    this.initCellWidth = [];
    this.fldSort = [];
    this._srdh = (dhx4.isIE && (document.compatMode != "BackCompat") ? 22 : 20);
    ...
    this.setSkin = function (name) {
        ...
        this.entBox.className = classname + " gridbox gridbox_" + name + (dhx4.isIE ? " isIE" : " isModern");
    ...
    this.init = function (fl) {
        ...
        if (dhx4.isIE && dhx4.isIE < 8 && document.body.style.msTouchAction == this.undefined) {
    ...
    this.setSizes = function () {
        ...
        var quirks = this.quirks = (dhx4.isIE && document.compatMode == "BackCompat");
    ...
    this.changeCursorState = function (ev) {
        ...
        var check = (ev.layerX || 0) + (((!dhx4.isIE) && (ev.target.tagName == "DIV")) ? el.offsetLeft : 0);
        if ((el.offsetWidth - (ev.offsetX || (parseInt(this.getPosition(el, this.hdrBox)) - check) * -1)) < (dhx4.isOpera ? 20 : 10)) {
    ...
    this._doClick = function (ev) {
        var selMethod = 0;
        var el = this.getFirstParentOfType(dhx4.isIE ? ev.srcElement : ev.target, "TD");
    ...
    this._doContClick = function (ev) {
        ...
        if (ev.button == 2 || (dhx4.isMacOS && ev.ctrlKey)) {
    ...
    this.doClick = function (el, fl, selMethod, show) {
        if (!this.selMultiRows) {
            selMethod = 0;
        } //block programmatical multiselecton if mode not enabled explitly
    ...
    this._onHeaderClick = function (e, el) {
        var that = this.grid;
        el = el || that.getFirstParentOfType(dhx4.isIE ? event.srcElement : e.target, "TD");
    ...
    this.getColumnLabel = function (cin, ind, hdr) {
        ...
        for (var i = 0; i < z.cells.length; i++) {
            if (z.cells[i]._cellIndexS == cin) {
                return (dhx4.isIE ? z.cells[i].innerText : z.cells[i].textContent);
            }
        }
        return "";
    };
    ...
    this.obj.ondblclick = function (e) {
        ...
        if (this.grid._dclE) {
            var row = this.grid.getFirstParentOfType((dhx4.isIE ? event.srcElement : e.target), "TR");
        ...
        if (dhx4.isOpera) {
            return false;
        } //block context menu for Opera 9+
    };
...
dhtmlXGridObject.prototype = {
    ...
    _hideShowColumn: function (ind, state) {
        ...
        if (state == "none") {
            ...
            if ((dhx4.isOpera && _OperaRv < 9) || dhx4.isSafari || (dhx4.isFirefox)) {
            ...
            if (this.hdr.rows[0].cells[ind]._oldWidth) {
                ...
                if (dhx4.isOpera || dhx4.isSafari || dhx4.isFirefox) {
    ...
    enableTooltips: function (list) {
        ...
        for (var i = 0; i < this._enbTts.length; i++) {
            this._enbTts[i] = dhx4.s2b(this._enbTts[i]);
        }
    }, //#}
    //#column_resize:06042008{
    ...
    _createHRow: function (data, parent) {
        ...
        for (var i = 0; i < data[0].length; i++) {
            ...
            if (this._hrrar && this._hrrar[i] && !dhx4.isIE) {
    ...
    _process_xml: function (xhr) {
        if (this._refresh_mode) {
            return this._refreshFromXML(xhr);
        }
        this._parsing = true;
        var top = dhx4.ajax.xmltop(this.xml.top, xhr);
        if (top.tagName != this.xml.top) {
            return;
        }
    ...
    render_dataset: function (min, max) {
        ...
        if (this._srnd && !this._fillers) {
            ...
            while (add_count > 0) {
                var add_step = (dhx4.isIE || dhx4.isFirefox) ? Math.min(add_count, block_size) : add_count;
...
dhtmlXGridObject.prototype._dp_init = function (dp) {
    ...
    this.attachEvent("onBeforeRowDeleted", function (rowId) {
        if (dp._silent_mode || (!this.rowsAr[rowId])) {
            return true;
        }
...
dhtmlXGridObject.prototype.load = function (url, call, type) {
    if (!this._colls_loaded && this.cellType) {
        var ar = [];
        for (var i = 0; i < this.cellType.length; i++) {
            if (this.cellType[i].indexOf("co") == 0 || this.cellType[i].indexOf("clist") == 0 || this._con_f_used[i] == 2) {
                ar.push(i);
            }
        }
        if (ar.length) {
...
eXcell_ch.prototype.setValue = function (val) {
    ...
    this.setCValue("<img src='" + this.grid.imgURL + "item_chk" + val + ".gif' onclick='new eXcell_ch(this.parentNode).changeState(true); (arguments[0]||event).cancelBubble=true; '>", this.cell.chstate);
};
...
function eXcell_txt(cell) {
    ...
    this.edit = function () {
        ...
        if (dhx4.isOpera) this.obj.onkeypress = function (ev) {
        ...
        if (this.list.parentNode != null) {
            if (this.editable) {
                ...
                if (ind && ind.text == this.obj.value)
                    this.setValue(this.list.value); else {
                    var combo = (this.cell._combo || this.grid.getCombo(this.cell._cellIndex));
                    var val = combo.values._dhx_find(this.obj.value);
                    if (val != -1) this.setValue(combo.keys[val]); else this.setValue(this.cell.combo_value = this.obj.value);
                }
            } else
...
eXcell_cp.prototype.setValue = function (val) {
    this.setCValue("<div style='width:100%;height:" + ((this.grid.multiLine ? "100%" : 23)) + ";background-color:" + (val || "") + ";border:0px;'>&nbsp;</div>", val);
...
function eXcell_img(cell) {
    ...
    this.getValue = function () {
        if (this.cell.firstChild.tagName == "IMG")
            return this.cell.firstChild.src + (this.cell.titFl != null ? "^" + this.cell._brval : ""); else if (this.cell.firstChild.tagName == "A") {
...
function eXcell_price(cell) {
    ...
    this.getValue = function () {
        if (this.cell.childNodes.length > 1)
            return this.cell.childNodes[1].innerHTML.toString()._dhx_trim(); else
            return "0";
    }
}
...
eXcell_dyn.prototype.setValue = function (val) {
    ...
    this.setCValue("<div class='grid_cell_dyn'><img src='" + this.grid.imgURL + "" + img + "'><span style='color:" + color + ";'>" + val + "</span></div>", val);
};
...
XCellObject.prototype.attachGrid = function () {
