class dhtmlXTabBar extends XCellTop {
    constructor(conf, mode) {
        ...
        this.conf = {
            ...
            tabsWidth: {
                ...
                dhx_skyblue: [35, 9],
                material: [44, 14]
            } // extra width for tabs/tab_with_close_icon, for tab width detect, since 4.2.1
        };
        ...
        if (conf != null && typeof(conf) == "object" && typeof(conf.tagName) == "undefined") {
            ...
            if (typeof(conf.skin) != "undefined") {
                this.conf.skin = conf.skin;
            }
            if (typeof(conf.mode) != "undefined") {
                this.conf.tabsMode = (conf.mode == "bottom" ? "bottom" : "top");
            }
            ...
            if (typeof(conf.json) != "undefined") {
                this.conf.autoload.xml = conf.json;
            } // new in 4.0
            if (typeof(conf.tabs) != "undefined") {
                this.conf.autoload.tabs = conf.tabs;
            }
            if (typeof(conf.onload) != "undefined") {
                this.conf.autoload.onload = conf.onload;
            } // new in 4.0
            if (typeof(conf.arrows_mode) != "undefined") {
                this.conf.autoload.arrows_mode = conf.arrows_mode;
            } // new in 4.1.2
            // deprecated from 4.0
        ...
        this.tabsArea.onclick = function (e) {
            ...
            while (t != null) {
                if (typeof(t._tabCloseId) != "undefined") {
                    if (that._onTabsAreaClick(t._tabCloseId) !== true) {
                        return;
                    }
        ...
        this.addTab = function (id, text, width, position, active, close) {
            ...
            tab._tabId = id;
            if (close) {
                tab.childNodes[1]._tabCloseId = id;
            } // close button
        ...
        this._getLabelWidth = function (text, close) {
            ...
            if (close == true) {
                w += this.conf.tabsWidth[this.conf.skin][1];
            }
            //
                    ...
                    if (this.t[id].conf.active) {
                        if (x < 0 || p.offsetWidth < w) {
                            k = {
                                d: 1,
                                id: id
                            }; // tab hidden on left side, move to right OR tab width less than space available
        ...
        this._moveTabs = function (d, tabId) {
            ...
            for (var q = 0; q < p.childNodes.length; q++) {
                ...
                if (id != null && this.t[id].conf.visible) {
                    tabs.push({
                        id: id,
                        w: this.t[id].tab.offsetWidth - this.conf.tabsOfs,
                        ind: i
                    });
                    if (id == tabId) {
                        tabInd = i;
            ...
            for (var q = 0; q < tabs.length; q++) {
                tabs[q].x = x;
                if (f == null && x >= 0 && x + tabs[q].w > 0) {
                    f = tabs[q];
                }
                if (x < totalSpace && x + tabs[q].w <= totalSpace) {
                    l = tabs[q];
                }
                x += tabs[q].w;
            }
            if (tabInd != null) {
                ...
                if (d > 0) {
                    ...
                    if (f == null) {
                        if (tabs.length > 0) {
                            t = tabs[tabs.length - 1];
                        }
                    } else {
                        if (f.ind > 0 && tabs.length >= f.ind) {
                            t = tabs[f.ind - 1];
                        }
                    }
                } else {
        ...
        this._getNearVisible = function (id, getFirst, mode) {
            if (mode == "first") {
                ...
                if (id == null || this.t[id] == null) {
                    return (getFirst ? this._getFirstVisible() : null);
                }
        ...
        this._showTab = function (id, activate) { // activate true/false
            ...
            if (this.t[id].conf.transActv == true) {
                if (this.t[id].conf.transMode == "show") {
                    return;
                }
            } else {
                if (this.t[id].conf.visible == true) {
                    return;
                }
            }
            // get next tab
        ...
        this._hideTab = function (id, activateId) { // activateId - tab to activate
            ...
            if (this.t[id].conf.transActv == true) {
                if (this.t[id].conf.transMode == "hide") {
                    return;
                }
            } else {
                if (this.t[id].conf.visible != true) {
        ...
        this.setSkin = function (skin) {
            ...
            for (var a in this.t) {
                ...
                if (this.t[a].conf.autowidth == true) {
                    ...
                    if (this.t[a].conf.visible) {
                        this.t[a].tab.style.width = this.t[a].conf.width + "px";
                    }
                }
            }
        ...
        this._initObj = function (data) {
            ...
            if (data.settings != null) {
                ...
                if (data.settings.hrefmode == "ajax") {
                    viaAjax = true;
                }
        ...
        this._xmlToObj = function (data) {
            ...
            if (r != null) {
                ...
                for (var a in {
                    ...
                    if (r.getAttribute(a) != null) {
                        obj.settings[a] = r.getAttribute(a);
                    }
                }
                // tabs
                ...
                for (var q = 0; q < t.length; q++) {
                    ...
                    for (var a in {
                        ...
                        if (t[q].getAttribute(a) != null) {
                            tab[a] = t[q].getAttribute(a);
                        }
                    }
                    // content
                    ...
                    if (cont != null) {
                        tab.content = "";
                        for (var w = 0; w < cont.childNodes.length; w++) {
                            tab.content += (cont.childNodes[w].nodeValue || "");
                        }
                    }
                    obj.tabs.push(tab);
                }
...
class dhtmlXTabBarCell extends XCellObject {
...
XCellObject.prototype.attachTabbar = function (conf) {
