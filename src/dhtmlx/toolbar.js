class dhtmlXToolbarObject {
    static get items(){
        return Object.defineProperty(this,'items',{
            value:{
                separator       : XToolbarSeparatorObject,
                text            : XToolbarTextObject,
                button          : XToolbarButtonObject,
                buttonSelect    : XToolbarButtonSelectObject,
                buttonInput     : XToolbarButtonInputObject,
                buttonTwoState  : XToolbarButtonTwoStateObject,
                slider          : XToolbarSliderObject
            }
        }).items
    }
    ...
    addButtonSelect(id, pos, text, opts, imgEnabled, imgDisabled, renderSelect, openAll, maxOpen, mode) {
        var options = [];
    ...
    _getPosition(id, getRealPosition) {
    ...
    _getIdByPosition(pos, retRealPos) {
    ...
    _addItemToStorage(data, pos) {
        ...
        if (type != "" && !!dhtmlXToolbarObject.items[type]) {
            ...
            this.objPull[this.idPrefix+id] = this._createItem(type,id,data);
    ...
    _createItem(type,id,data){
        return new (dhtmlXToolbarObject.items[type])(this, id, data);
    }
    setSkin(skin, onlyIcons) {
        if (onlyIcons === true) {
    ...
    _isListOptionExists(listId, optionId) {
    ...
    setListOptionUserData(listId, optionId, name, value) {
        // is exists?
    ...
    getListOptionUserData(listId, optionId, name) {
        // is exists?
    ...
    _improveTerraceButtonSelect(id, state) {
...
class XToolbarSeparatorObject {
    constructor(that, id, data) {
        //
...
class XToolbarButtonObject {
    constructor(that, id, data) {
        ...
        //
        var img ="<i class='material-icons'>"+this.img+"</i>"//(that.conf.icons_css?"<i class='"+that.conf.icons_path+this.img+"'></i>":"<img src='"+that.conf.icons_path+this.img+"'>");
...
class XToolbarButtonSelectObject {
    constructor(that, id, data) {
        this.id = that.idPrefix+id;
        ...
        this.arw.innerHTML = `<i class="material-icons">keyboard_arrow_down</i>`;
...
class XToolbarButtonInputObject {
    constructor(that, id, data) {
        //
        this.id = that.idPrefix+id;
        this.obj = document.createElement("DIV");
...
class XToolbarButtonTwoStateObject {
    constructor(that, id, data) {
        this.id = that.idPrefix+id;
        this.state = (data.enabled!=null?false:true);
...
XCellObject.prototype._createNode_toolbar = function(obj, type, htmlString, append, node) {
...
XCellObject.prototype.attachToolbar = function(conf) {
...
XCellObject.prototype.detachToolbar = function() {
...
XCellObject.prototype.showToolbar = function() {
	this._mtbShowHide("toolbar", "");
};
XCellObject.prototype.hideToolbar = function() {
	this._mtbShowHide("toolbar", "none");
};
XCellObject.prototype.getAttachedToolbar = function() {
	return this.dataNodes.toolbar;
};
