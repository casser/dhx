...
(function () {
    for (var a in dhtmlXForm.prototype.items.select) {
        if (!dhtmlXForm.prototype.items.multiselect[a]) {
            dhtmlXForm.prototype.items.multiselect[a] = dhtmlXForm.prototype.items.select[a];
        }
    }
})();
dhtmlXForm.prototype.items.input = {
    render: function (item, data) {
        ...
        this.doAttachEvents(item);
        if (ta) {
            item.childNodes[item._ll ? 1 : 0].childNodes[0].rows = Number(data.rows) + (window.dhx4.isIE6 ? 1 : 0);
        }
